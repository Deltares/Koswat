...
class TestProfileZoneCalculator:
    ...
    @pytest.mark.parametrize(
        "points_profile, points_reinforcement, expected_zones",
        [
            pytest.param([], [], (math.nan, math.nan), id="Only Zone B calculated"),
            pytest.param(
                [], [], (math.nan, math.nan), id="Zone A and Zone B calculated"
            ),
        ],
    )
    def test_given_reinforcement_profile_when_calculate_then_expected_zones(
        self,
        points_profile: list[tuple[float]],
        ...
        _reinforcement_profile = reinforcement_profile_builder(
            points_profile, points_reinforcement
        )
