...
def _validate_test_section_scenario(
    ...
    assert isinstance(input_fom, KoswatJsonFomProtocol)
...
class TestKoswatSectionScenariosJsonFom:
    ...
    def test_from_config(self):
        ...
        # 2. Run test
        _config_fom = KoswatSectionScenariosJsonFom.from_config(_input_dict)

        # 3. Verify expectations
        assert isinstance(_config_fom, KoswatSectionScenariosJsonFom)
        assert isinstance(_config_fom, KoswatJsonFomProtocol)
        assert _config_fom.scenario_dike_section == ""
        assert len(_config_fom.section_scenarios) == 1
        _validate_test_section_scenario(
            _config_fom.section_scenarios[0], "test_section"
        )
