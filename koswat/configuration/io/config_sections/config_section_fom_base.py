...
class ConfigSectionFomBase(FileObjectModelProtocol, abc.ABC):
    ...
    def _set_bool_values(
        ...
        for key, attr in mappings.items():
            ...
            setattr(
                self,
                attr,
                bool(config.get(key, default)) if _input_value != default else default,
            )
    ...
    def _set_surtax_factor_values(
        ...
        for key, attr in mappings.items():
            _input_value = config.get(key, default.name if default else None)
            setattr(
                self,
                attr,
                SurtaxFactorEnum[_input_value.upper()] if _input_value else default,
            )
