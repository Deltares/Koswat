...
class AnalysisSectionFom(KoswatJsonFomProtocol):
    ...
    export_measures_png: bool
    export_layers_png: bool
    export_shapefiles: bool

    @classmethod
    def from_config(
        cls, input_config: dict[str, Any], parent_path: Path
    ) -> "AnalysisSectionFom":
        ...
        def resolve_path(input_path: str) -> Path:
            if (
                PureWindowsPath(input_path).is_absolute()
                or PurePosixPath(input_path).is_absolute()
            ):
        ...
        return cls(
            dike_section_location_shp_file=resolve_path(
