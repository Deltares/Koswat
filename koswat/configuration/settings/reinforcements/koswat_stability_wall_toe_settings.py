...
@dataclass
class KoswatStabilityWallToeSettings(KoswatConfigProtocol):
    """
    Settings related to Stability wall (toe) reinforcement
    """
    ...
    def set_defaults(
        self,
        other: "KoswatStabilityWallToeSettings",
    ) -> "KoswatStabilityWallToeSettings":
        """
        Add the defaults from another KoswatStabilityWallToeSettings instance to this instance, if not set.

        Args:
            other (KoswatStabilityWallToeSettings): The other instance to get defaults from.

        Raises:
            TypeError: If the other instance has a different type.

        Returns:
            KoswatStabilityWallToeSettings: The instance with defaults set.
        """
        if not isinstance(other, KoswatStabilityWallToeSettings):
            raise TypeError(
                "Can only merge with another KoswatStabilityWallToeSettings instance."
            )
