...
import numpy as np
import pandas as pd
from ipyleaflet import DrawControl, GeoData, LayersControl, LegendControl, Map, basemaps
...
def maak_interactieve_kaart(pad):
    ...
    geo_data = GeoData(
        ...
        style={"color": "red", "fillColor": "black", "fillOpacity": 0.1, "weight": 2},
        name="Dijksecties",
    )
    ...
    draw_control = DrawControl(
        rectangle={
            "shapeOptions": {"color": "#0000FF", "weight": 2, "fillOpacity": 0.1}
        },
        circlemarker={},
        circle={},
        polyline={},
    ...
    def handle_draw(target, action, geo_json):
        ...
        selected["gdf"] = (
            pd.concat([selected["gdf"], new_selection])
            .drop_duplicates(
                subset=dijksecties.columns.difference(["geometry"]).tolist()
            )
