...
def plot_profile(filename, pad):
    ...
    def profilepoints(jsonfile):
        ...
        wx0 = x0
        wy0 = max(
            Dijkprofiel["Kruin_Hoogte"] - 0.5, Dijkprofiel["Buiten_Maaiveld"] + 0.1
        )
    ...
    with plt.xkcd(randomness=randomness_level):
        ...
        ax.plot(
            dike_line_x, dike_line_y, color="#B7D167", linewidth=2, label="Dijkprofiel"
        )
        ax.plot(
            ref_line_x,
            ref_line_y,
        ...
        ax.plot(
            dike_line_x, dike_line_y, color="#38312c", linewidth=2, label="Dijkprofiel"
        )
        ...
        Dijkprofiel = data["Dijkprofiel"]
        text_str = "\n".join(
            [f"{key}: {value:.2f}" for key, value in Dijkprofiel.items()]
        )
        ...
        ax.text(
            ...
            ha="left",
            bbox=dict(
                boxstyle="round,pad=0.5", facecolor="whitesmoke", edgecolor="gray"
            ),
        )
